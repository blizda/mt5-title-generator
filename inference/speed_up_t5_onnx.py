...
def create_t5_encoder_decoder(model="google/mt5-small", model_st_dict=None):
    ...
    if isinstance(model, str):
        if model_st_dict:
            model = MT5ForConditionalGeneration.from_pretrained(model, state_dict=torch.load(model_st_dict))
        else:
...
def generate_onnx_representation(model, encoder_path, lm_path, model_st_dict):
    ...
    simplified_encoder, decoder, lm_head = create_t5_encoder_decoder(model, model_st_dict)
...
class OnnxT5(GenerationMixin):
    def __init__(self, model_name_or_path, onnx_path, model_st_d=None):

        self.model_name_or_path = Path(model_name_or_path)
        self.model_st_d = model_st_d
        ...
        self.config = MT5Config.from_pretrained(model_name_or_path)
        self.device = 'cpu'

        if model_st_d:
            decoder = MT5ForConditionalGeneration.from_pretrained(model_name_or_path, state_dict=torch.load(model_st_d)).decoder
        else:
            decoder = MT5ForConditionalGeneration.from_pretrained(model_name_or_path).decoder
    ...
    def _export_onnx_graph(self):
        ...
        generate_onnx_representation(
            self.model_name_or_path.as_posix(), self.encoder_path.as_posix(), self.lm_head_path.as_posix(), self.model_st_d,
        )
